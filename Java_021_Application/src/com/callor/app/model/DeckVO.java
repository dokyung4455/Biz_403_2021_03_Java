...
public class DeckVO {
	...
	// 기본 생성자(super constructor) : 매개변수가 없는 생성자
	// new DeckVO() 처럼 호출
	// class를 정의하고 아무런 생성자를 만들지 않으면 기본 생성자는 자동으로 만들어져
	// 언제든지 객체를 생성할때 호출할 수 있다.
	
	// 하지만, 매개변수가(한개라도) 있는 생성자를 만들면 기본 생성자는 만들어지지 않는다.
	public DeckVO() {
		
	}
	
	// 필드(멤버)변수를 매개변수로 갖는 생성자 (Field Constructor)
	// new DeckVO(값 ...) 처럼 호출
