 ...
 * 
 * public : 모두에게 개방한다, 모두에게 공개한다, 누구나 접근(읽기,쓰기,실행)을 하게한다.
 * 			Class obj = new Class()
 * 			obj.method();
 * 			obj.변수 = 100;
 * 
...
public class ScoreServiceV1 {
	...
	/* 멤버변수를 protected로 선은을 해 두면 현재 클래스(V1)를 상속받은 자식 클래스에서
	...
	public Integer inputScore() {
		...
		int intKor = 0;
		while (true) {
			System.out.println(strNum + " 번의 국어점수를 입력하세요.(stop : -1)");
			System.out.print(">> ");
			intKor = scan.nextInt();
			if (intKor == -1) {
				return -1;
			} else if(intKor < 0 || intKor > 100) {
				System.out.println("점수는 0 ~ 100까지 입력할 수 있습니다.");
				continue;
			}
			break;
		}
	...
	public void printScore() {
		...
		for (int i = 0; i < scoreList.size(); i++) {
			...
			// scoreList의 i 번째 저장된 값을 읽어서 ScoreVO형 sVO 객체에 담기
	...
	public void printHeader() {
		System.out.println(" * 빛나지마라 고교 성적처리 V1");
