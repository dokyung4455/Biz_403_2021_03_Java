...
public class App_09 {
	...
	public static void main(String[] args) {
		...
		String[] strNames = {"홍길동","이몽룡","성춘형","장녹수","임꺽정","장보고"};
		...
		for(int i = 0 ; i < strNames.length ; i++) {
			...
			intKor[i] = rnd.nextInt(51) + 50; // 각자 국어 점수
			intEng[i] = rnd.nextInt(51) + 50; // 각자 영어 점수
			intMath[i] = rnd.nextInt(51) + 50; // 각자 수학 점수
		}
		
		for(int i = 0 ; i < strNames.length ; i++) {
			intTotal[i] = intKor[i]; // 국어점수 포함
			intTotal[i] += intEng[i]; // 국어 + 영어 점수 포함
			intTotal[i] += intMath[i]; // 국어 + 영어 + 수학 점수 포함
			
			floatAvg[i] = intTotal[i] / 3.0f; // 총 인원 평균
		}
